...
We can check if hemibiotrophic species are in fact enriched in the ground, and obligate biotrophs in the canopy. We do a poisson regression:

```{r}
OTU_Table_hemi = OTU_Table[,TAX$Lifestyle == "hemibiotroph"]
OTU_Table_obli = OTU_Table[,TAX$Lifestyle == "obligate biotroph"]

data_hemi = data.frame(Abundance = rowSums(OTU_Table_hemi), 
                       Microhabitat = SampleMetadata$Microhabitat, 
                       TreeSpecies = SampleMetadata$TreeSpecies, 
                       Stratum = SampleMetadata$Stratum, 
                       Lifestyle = "hemibiotroph")

data_obli = data.frame(Abundance = rowSums(OTU_Table_obli), 
                       Microhabitat = SampleMetadata$Microhabitat, 
                       TreeSpecies = SampleMetadata$TreeSpecies, 
                       Stratum = SampleMetadata$Stratum, 
                       Lifestyle = "obligate biotroph")

summary(glm(data = data_hemi, Abundance ~ Microhabitat, family = "poisson"))
summary(glm(data = data_obli, Abundance ~ Microhabitat, family = "poisson"))

plot(data_obli$Abundance ~ data_obli$Microhabitat)
plot(data_hemi$Abundance ~ data_hemi$Microhabitat)

data_both = rbind(data_hemi, data_obli)
data_both$Microhabitat = factor(data_both$Microhabitat, 
                                levels = c("Arboreal Soil", "Bark", "Deadwood", 
                                           "Fresh Leaves", "Hypnum", "Lichen", 
                                           "Orthotrichum", "Leaf Litter", 
                                           "Soil"))

g = ggplot(data_hemi, aes(x = Microhabitat, y = Abundance)) + 
  geom_point() + 
  geom_smooth(method = "lm", formula = Abundance ~ Microhabitat)
```
